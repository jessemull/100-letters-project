  ...
  return (
    <div className="p-4 bg-fixed bg-center bg-[repeating-linear-gradient(45deg,_blue_0,_blue_16px,_red_16px,_red_32px,_white_32px,_white_48px)] min-h-screen w-full">
      <div className="flex w-full bg-white text-gray-800 font-merriweather min-h-[calc(100vh-16px)]">
        <div className="w-full py-4 px-4 sm:py-16 sm:px-24">
          <div className="sm:hidden mb-6 sm:mb-4">
            <label htmlFor="tab-select" className="sr-only">
              Select a section
            </label>
            <select
              className="w-full pl-3 pr-3 py-3 rounded-xl border-2 border-black shadow-sm bg-white text-xl font-bold appearance-none"
              id="tab-select"
          ...
          <TabGroup selectedIndex={selectedIndex} onChange={setSelectedIndex}>
            <TabList className="hidden sm:flex space-x-2 mb-6 border-b-2 border-black">
              {tabs.map((tab) => (
                <Tab
                  className={`px-4 py-2 font-semibold text-xl transition ${
            ...
            <div className="mb-6">
              <div className="relative w-full">
                <Search
                  className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-700 w-6 h-6 sm:w-5 sm:h-5"
                  strokeWidth={3}
                />
                <input
